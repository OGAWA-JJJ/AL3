...
{
	auto d3dDevice = DirectXImportant::dev.Get();
	m_width = w;
	m_height = h;
...
bool RenderTarget::CreateRenderTargetTexture(ID3D12Device*& d3dDevice, int w, int h, int mipLevel, int arraySize, DXGI_FORMAT format, float clearColor[4])
{
...
bool RenderTarget::CreateDepthStencilTexture(ID3D12Device*& d3dDevice, int w, int h, DXGI_FORMAT format)
{
